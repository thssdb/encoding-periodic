...
class DescendingSeparateEncoder(Encoder):
    ...
    def __init__(
        ...
        self.gvle_only = gvle_only
        self.dbp_only = dbp_only
    ...
    def encode_stream(self, stream: BitStream, data: NDArray[np.int64]):
        ...
        if self.is_given_beta:
            optimal_beta = self.given_beta
        else:
            estimate_encoding_length = (
                self.frequency_estimate(
                    frequency_value_real, self.min_beta, self.max_beta
                )
                + self.frequency_estimate(
    ...
    def decode(self, stream: BitStream) -> NDArray[np.int64]:
        ...
        if self.gvle_only:
            frequency_value_quantized_real = variable_length_decreasing_decode(
                stream, save_length=False, given_length=(n // 2 + 1)
            )
            frequency_value_quantized_imag = variable_length_decreasing_decode(
                stream, save_length=False, given_length=(n // 2 + 1)
            )
        elif self.dbp_only:
        ...
        if self.simple_residual:
            ...
            residual = separate_storage_decode(
                stream, save_length=False, given_length=n
            )
        data = (
    ...
    def get_name(self) -> str:
        return "Descending-Separate-Alblation"

    def exp(self, data: NDArray[np.int64]) -> DataFrame:
        result = super().exp(data)
        result["is_given_beta"] = self.is_given_beta
        result["given_beta"] = self.given_beta
        result["gvle_only"] = self.gvle_only
        result["dbp_only"] = self.dbp_only
